...
void surface_push_quad(
    Surface* surface, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4,
    Clay_Color color
) {
    SurfaceCommand* cmd = push_command(surface);
    if (!cmd)
...
void surface_push_text(
    Surface* surface, int x, int y, const char* text, int fontId, int fontSize, Clay_Color color
) {
    SurfaceCommand* cmd = push_command(surface);
    if (!cmd)
        return;
...
static Clay_BoundingBox clamp_to_surface(Surface* surface, int x, int y, int w, int h) {
    ...
    return (Clay_BoundingBox){
        (float)(surface->originX + x1), (float)(surface->originY + y1), (float)clampedW,
        (float)clampedH
    };
}
...
void surface_render(Display* display, Surface* surface) {
    for (int i = 0; i < surface->count; i++) {
        ...
        case SURFACE_CMD_QUAD: {
            display_fill_quad(
                display, surface->originX + cmd->quad[0][0], surface->originY + cmd->quad[0][1],
                surface->originX + cmd->quad[1][0], surface->originY + cmd->quad[1][1],
                surface->originX + cmd->quad[2][0], surface->originY + cmd->quad[2][1],
                surface->originX + cmd->quad[3][0], surface->originY + cmd->quad[3][1], cmd->color
            );
            break;
        }

        case SURFACE_CMD_CIRCLE: {
            display_fill_circle(
                display, surface->originX + cmd->x, surface->originY + cmd->y, cmd->w, cmd->color
            );
            break;
        }

        case SURFACE_CMD_TEXT: {
            if (cmd->x < 0 || cmd->y < 0 || cmd->x >= surface->width || cmd->y >= surface->height)
                continue;

            Clay_BoundingBox box = {
                (float)(surface->originX + cmd->x), (float)(surface->originY + cmd->y), 0, 0
            };
