...
local ballSpeed = 0
local BALL_SPEED_FRACTION = 0.3
local PADDLE_SPEED_FRACTION = 0.005
...
local function resetLayout()
    ...
    ballSpeedX = w * BALL_SPEED_FRACTION
    ballSpeedY = h * BALL_SPEED_FRACTION
    paddleSpeed = h * PADDLE_SPEED_FRACTION
    aiSpeed = h * AI_SPEED_FRACTION
end

local function resetBall(direction)
    ballX = draw.width() / 2
    ballY = draw.height() / 2
    ballXvel = ballSpeedX * direction
    ballYvel = (p1score + p2score) % 2 == 0 and ballSpeedY * 0.5 or -ballSpeedY * 0.5
end
