...
static SDL_Color to_sdl_color(Clay_Color colour) {
...
bool display_init(Display* display, CyanData* data) {
    ...
    if (!SDL_CreateWindowAndRenderer(
            "Watch", display->width, display->height, SDL_WINDOW_RESIZABLE, &backend->window,
            &backend->renderer
        )) {
        return false;
    }
    ...
    SDL_Texture* empty_tab_tex = SDL_CreateTexture(
        backend->renderer, SDL_PIXELFORMAT_XRGB8888, SDL_TEXTUREACCESS_STATIC, ICON_EMPTY_TAB_WIDTH,
        ICON_EMPTY_TAB_HEIGHT
    );
    ...
    SDL_Texture* full_tab_tex = SDL_CreateTexture(
        backend->renderer, SDL_PIXELFORMAT_XRGB8888, SDL_TEXTUREACCESS_STATIC, ICON_FULL_TAB_WIDTH,
        ICON_FULL_TAB_HEIGHT
    );
...
void display_clear(Display* display, Clay_Color colour) {
    SdlBackend* backend = display->backend;
    SDL_Color c = to_sdl_color(colour);
...
void display_fill_quad(
    Display* display, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4,
    Clay_Color colour
) {
    SdlBackend* backend = display->backend;
...
void display_fill_circle(
    Display* display, int centre_x, int centre_y, int radius, Clay_Color colour
) {
    SdlBackend* backend = display->backend;
    SDL_FColor c = {colour.r / 255.0f, colour.g / 255.0f, colour.b / 255.0f, colour.a / 255.0f};

    SDL_Vertex vertices[CIRCLE_SEGMENTS + 1];
    int indices[CIRCLE_SEGMENTS * 3];

    vertices[0] = (SDL_Vertex){.position = {(float)centre_x, (float)centre_y}, .color = c};
    for (int i = 0; i < CIRCLE_SEGMENTS; i++) {
        float angle = (2.0f * (float)M_PI * (float)i) / (float)CIRCLE_SEGMENTS;
        vertices[i + 1] = (SDL_Vertex){
            .position = {centre_x + cosf(angle) * radius, centre_y + sinf(angle) * radius},
            .color = c
        };
    }
    for (int i = 0; i < CIRCLE_SEGMENTS; i++) {
    ...
    SDL_RenderGeometry(
        backend->renderer, NULL, vertices, CIRCLE_SEGMENTS + 1, indices, CIRCLE_SEGMENTS * 3
    );
}

void display_draw_border(Display* display, Clay_BoundingBox box, Clay_BorderRenderData border) {
    SdlBackend* backend = display->backend;
    SDL_Color c = to_sdl_color(border.color);
    SDL_SetRenderDrawColor(backend->renderer, c.r, c.g, c.b, c.a);
    ...
    if (border.width.bottom > 0) {
        SDL_FRect bottom = {
            box.x, box.y + box.height - border.width.bottom, box.width, (float)border.width.bottom
        };
    ...
    if (border.width.right > 0) {
        SDL_FRect right = {
            box.x + box.width - border.width.right, box.y, (float)border.width.right, box.height
        };
...
void display_draw_text(Display* display, Clay_BoundingBox box, Clay_TextRenderData text) {
    SdlBackend* backend = display->backend;
    SDL_Color c = to_sdl_color(text.textColor);
    gfx_text_draw(
        backend->renderer, text.fontSize, text.stringContents.chars, text.stringContents.length,
        (int)box.x, (int)box.y, c
    );
}
...
bool display_capture_screenshot(Display* display, const char* path) {
    ...
    if (!surface) {
        cyan_log(VERBOSE_LOW, "[Screenshot] Failed to read pixels: %s", SDL_GetError());
        return false;
    }

    bool ok = IMG_SavePNG(surface, path);
    if (!ok) {
        cyan_log(VERBOSE_LOW, "[Screenshot] Failed to save PNG: %s", SDL_GetError());
    }
...
void display_loading_screen(Display* display, float progress) {
    ...
    if (!loadingIconTexture) {
        loadingIconTexture = SDL_CreateTexture(
            backend->renderer, SDL_PIXELFORMAT_XRGB8888, SDL_TEXTUREACCESS_STATIC, ICON_CYAN1_WIDTH,
            ICON_CYAN1_HEIGHT
        );
        SDL_UpdateTexture(
            loadingIconTexture, NULL, ICON_CYAN1, ICON_CYAN1_WIDTH * sizeof(uint32_t)
        );
    ...
    display_draw_image(
        ...
        (Clay_ImageRenderData){.imageData = loadingIconTexture}
    );
